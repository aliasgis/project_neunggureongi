...
import csv,html,json,uuid,os,re,threading,time,hashlib,importlib
...
def db_engine(connection_id=''):
 ...
 if key not in _DB_ENGINES:
  ...
  _DB_ENGINES[key]=create_engine(url,pool_pre_ping=True,pool_timeout=5,connect_args={'connect_timeout':5,'options':'-c statement_timeout=60000'})
...
class WpsScriptInput(BaseModel):
 filename:str
 source:str
...
@app.get('/api/wps/scripts')
def wps_script_catalog():
 scripts=[]
 for path in sorted((BASE/'wps').glob('*.py')):
  if path.name.startswith('_') or path.name in ('registry.py','__init__.py'):continue
  scripts.append({'filename':path.name,'source':path.read_text(encoding='utf-8')})
 return {'scripts':scripts}

@app.put('/api/wps/scripts/{filename}')
def save_wps_script(filename:str,data:WpsScriptInput):
 if filename!=data.filename or not re.fullmatch(r'[a-z][a-z0-9_]*\.py',filename):
  raise HTTPException(400,'파일명은 영문 소문자로 시작하고 소문자, 숫자, 밑줄만 사용할 수 있습니다.')
 if filename in ('registry.py','__init__.py') or filename.startswith('_'):
  raise HTTPException(400,'예약된 WPS 파일명입니다.')
 if len(data.source.encode('utf-8'))>512*1024:
  raise HTTPException(413,'WPS 스크립트는 512KB 이하여야 합니다.')
 try:compile(data.source,filename,'exec')
 except SyntaxError as error:
  raise HTTPException(400,f'Python 문법 오류 ({error.lineno}행): {error.msg}')
 target=BASE/'wps'/filename;temporary=target.with_suffix('.py.tmp');previous=target.read_bytes() if target.exists() else None
 try:
  temporary.write_text(data.source,encoding='utf-8');temporary.replace(target)
  importlib.invalidate_caches()
  processes=discover_processes()
  registered=[item['metadata'] for item in processes.values() if item['metadata']['source']==f'wps/{filename}']
  if not registered:raise ValueError('PROCESS 메타데이터와 execute(layer, parameters, context) 함수를 정의해야 합니다.')
 except Exception as error:
  temporary.unlink(missing_ok=True)
  if previous is None:target.unlink(missing_ok=True)
  else:target.write_bytes(previous)
  importlib.invalidate_caches()
  raise HTTPException(400,f'WPS 등록 실패: {error}')
 return {'status':'registered','filename':filename,'processes':registered}
...
async def wps(r:Request):
 ...
 if op=='EXECUTE':
  ...
  try:
   ...
   process=discover_processes().get(pid)
   if not process:raise ValueError(f'Unknown WPS process: {pid}')
   requires_layer=process['metadata'].get('requires_layer',True)
   layer=None
   if requires_layer and input_mode=='WKT':
...
<main><div class="bar"><span class="count" id="count">불러오는 중…</span><div><button class="btn ghost" onclick="openPostgisConnections()">PostGIS 연결</button> <button class="btn ghost" onclick="openWpsEditor()">WPS Python</button> <button class="btn ghost" id="wpsButton" onclick="openWps()">WPS 작업</button> <button class="btn ghost" id="transformButton" onclick="openTransform()">좌표 변환</button> <button class="btn primary" id="addLayerButton" onclick="openForm()">+ 레이어 추가</button></div></div>
...
<dialog id="wpsEditorDialog" class="previewModal"><form class="modal" id="wpsEditorForm"><h2>WPS Python 스크립트</h2><div class="grid"><label>기존 스크립트<select id="wpsScriptList" onchange="selectWpsScript()"><option value="">새 스크립트</option></select></label><label>파일명 *<input id="wpsScriptFilename" required pattern="[a-z][a-z0-9_]*\.py" placeholder="my_process.py"><span class="hint">저장과 동시에 WPS 서비스에 등록됩니다.</span></label><label class="full">Python 소스<textarea id="wpsScriptSource" rows="24" required spellcheck="false"></textarea></label><div class="full"><span class="hint uploadState" id="wpsScriptState"></span></div></div><div class="foot"><button type="button" class="btn ghost" onclick="newWpsScript()">새 스크립트</button><button type="button" class="btn ghost" onclick="wpsEditorDialog.close()">닫기</button><button class="btn primary" id="wpsScriptSave">저장 및 서비스 등록</button></div></form></dialog>
...
let layers=[],wpsScripts=[],wpsPreviewLayers={},postgisConnections=[],postgisTables=[],editing=null,editingConnection=null,map=null,wmsLayer=null,wfsLayer=null,previewMbr=null,activePreview=null,activePreviewMode='WMS',activePreviewFilter='',wpsObjectUrl=null,wpsProcesses=[],previewRefreshTimer=null,previewRequestSeq=0;const dialog=document.querySelector('#dialog'),previewDialog=document.querySelector('#previewDialog'),transformDialog=document.querySelector('#transformDialog'),wpsDialog=document.querySelector('#wpsDialog'),wpsEditorDialog=document.querySelector('#wpsEditorDialog'),postgisDialog=document.querySelector('#postgisDialog'),form=document.querySelector('#form'),transformForm=document.querySelector('#transformForm'),wpsForm=document.querySelector('#wpsForm'),wpsEditorForm=document.querySelector('#wpsEditorForm'),postgisForm=document.querySelector('#postgisForm');
...
const wpsScriptTemplate=['from wps.registry import WpsResult','','PROCESS = {','    "id": "custom.example",','    "title_ko": "사용자 프로세스",','    "title_en": "Custom process",','    "description": "사용자 정의 WPS 프로세스",','    "layer_types": ["raster"],','    "parameters": [],','    "output": "json",','}','','','def execute(layer, parameters, context):','    return WpsResult("json", {"layer": layer["name"], "status": "ok"})',''].join('\\n');
function newWpsScript(){document.querySelector('#wpsScriptList').value='';document.querySelector('#wpsScriptFilename').value='';document.querySelector('#wpsScriptFilename').readOnly=false;document.querySelector('#wpsScriptSource').value=wpsScriptTemplate;document.querySelector('#wpsScriptState').textContent='새 파일명을 입력하고 PROCESS와 execute를 구현하세요.'}
function selectWpsScript(){const filename=document.querySelector('#wpsScriptList').value,item=wpsScripts.find(script=>script.filename===filename);if(!item){newWpsScript();return}document.querySelector('#wpsScriptFilename').value=item.filename;document.querySelector('#wpsScriptFilename').readOnly=true;document.querySelector('#wpsScriptSource').value=item.source;document.querySelector('#wpsScriptState').textContent='기존 스크립트를 편집하고 저장하면 즉시 다시 등록됩니다.'}
async function openWpsEditor(){const state=document.querySelector('#wpsScriptState');state.textContent='스크립트 목록을 불러오는 중…';if(!wpsEditorDialog.open)wpsEditorDialog.showModal();try{const response=await fetch('/api/wps/scripts'),data=await response.json();if(!response.ok)throw new Error(data.detail||data.error||'스크립트 조회 실패');wpsScripts=data.scripts||[];document.querySelector('#wpsScriptList').innerHTML='<option value="">새 스크립트</option>'+wpsScripts.map(script=>`<option value="${esc(script.filename)}">${esc(script.filename)}</option>`).join('');newWpsScript()}catch(error){state.textContent='오류: '+error.message}}
wpsEditorForm.onsubmit=async event=>{event.preventDefault();const button=document.querySelector('#wpsScriptSave'),filename=document.querySelector('#wpsScriptFilename').value.trim(),source=document.querySelector('#wpsScriptSource').value,state=document.querySelector('#wpsScriptState');button.disabled=true;state.textContent='검증 및 등록 중…';try{const response=await fetch('/api/wps/scripts/'+encodeURIComponent(filename),{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({filename,source})}),data=await response.json();if(!response.ok)throw new Error(data.detail||data.error||'WPS 등록 실패');state.textContent=`등록 완료: ${data.processes.map(item=>item.id).join(', ')}`;notify('WPS 서비스에 등록했습니다.');await openWpsEditor();document.querySelector('#wpsScriptList').value=filename;selectWpsScript()}catch(error){state.textContent='오류: '+error.message}finally{button.disabled=false}};
...
async function previewLayer(name){const l=layers.find(x=>x.name===name)||wpsPreviewLayers[name];if(!l)return alert('레이어를 찾을 수 없습니다.');if(!window.L)return alert('Leaflet을 불러오지 못했습니다. 인터넷 연결을 확인해 주세요.');activePreview=name;activePreviewFilter='';document.querySelector('#previewFilter').value='';document.querySelector('#previewFilterHint').textContent='';if(l.type!=='raster')fetch('/api/layers/'+encodeURIComponent(name)+'/fields').then(response=>response.json()).then(data=>{if(data.fields)document.querySelector('#previewFilterHint').textContent='사용 가능한 필드: '+data.fields.join(', ')}).catch(()=>{});
 ...
 setTimeout(()=>map.invalidateSize(),0);await showLayerMbr(l);await changePreviewService()}
async function showLayerMbr(layer){if(previewMbr&&map){map.removeLayer(previewMbr);previewMbr=null}try{const response=await fetch('/api/layers/'+encodeURIComponent(layer.name)+'/bounds'),data=await response.json();if(!response.ok)throw new Error(data.detail||'MBR 조회 실패');const b=data.bounds,bounds=[[b[1],b[0]],[b[3],b[2]]];previewMbr=L.rectangle(bounds,{color:'#f59e0b',weight:2,dashArray:'7 5',fill:false,interactive:false}).addTo(map);map.fitBounds(bounds,{padding:[28,28],maxZoom:17});document.querySelector('#previewMeta').textContent=`${layer.name} · MBR 경계로 이동 완료`;return b}catch(error){document.querySelector('#previewMeta').textContent=`MBR 이동 실패 · ${error.message}`;return null}}
...
async function changePreviewService(){if(!activePreview||!map)return;const l=layers.find(x=>x.name===activePreview)||wpsPreviewLayers[activePreview],mode=document.querySelector('#previewService').value;activePreviewMode=mode;document.querySelector('#previewFilterBar').style.display=l.type!=='raster'&&['WMS','WMS_TILED'].includes(mode)?'flex':'none';clearPreviewLayers();document.querySelector('#previewMeta').textContent=`${l.name} · ${l.type} · ${mode} 미리보기`;
 ...
 else if(mode==='WMS_TILED'){const wmsOptions={layers:l.name,format:'image/png',transparent:true,version:'1.1.1',tileSize:256,opacity:.85,updateWhenZooming:false,updateWhenIdle:true,keepBuffer:1};if(activePreviewFilter)wmsOptions.CQL_FILTER=activePreviewFilter;wmsLayer=L.tileLayer.wms(location.origin+'/wms',wmsOptions).addTo(map);try{const r=await fetch('/api/layers/'+encodeURIComponent(l.name)+'/bounds');if(!r.ok)throw new Error();const b=(await r.json()).bounds;map.fitBounds([[b[1],b[0]],[b[3],b[2]]],{padding:[24,24],maxZoom:17});document.querySelector('#previewMeta').textContent=`${l.name} · ${l.type} · WMS 타일 방식${activePreviewFilter?' · '+activePreviewFilter:''}`}catch{map.setView([36.4,127.8],7);document.querySelector('#previewMeta').textContent+=' · 영역 자동 이동 실패'}}
...
previewDialog.addEventListener('close',()=>{activePreview=null;previewRequestSeq++;if(previewRefreshTimer)clearTimeout(previewRefreshTimer);clearPreviewLayers();if(previewMbr&&map){map.removeLayer(previewMbr);previewMbr=null}});
