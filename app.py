...
def publish_wps_raster(result,process_id,source_name):
  ...
  try:
   ...
   if result.kind=='file':
    if publish and result.media_type in ('image/tiff','image/geotiff'):
     result_id,result_layer,expires_at=publish_wps_raster(result,pid,layer['name'])
...
wpsForm.onsubmit=async event=>{event.preventDefault();const button=document.querySelector('#wpsSubmit'),result=document.querySelector('#wpsResult'),download=document.querySelector('#wpsDownload'),preview=document.querySelector('#wpsPreview');button.disabled=true;download.style.display='none';preview.style.display='none';preview.dataset.layer='';if(wpsObjectUrl){URL.revokeObjectURL(wpsObjectUrl);wpsObjectUrl=null}result.value='처리 중…';try{const process=document.querySelector('#wpsProcess').value,params=buildWpsParams();const response=await fetch('/wps?'+params),contentType=response.headers.get('content-type')||'';if(contentType.includes('json')){const data=await response.json();if(!response.ok)throw new Error(data.error||'WPS 실행 실패');result.value=JSON.stringify(data,null,2);if(data.layer){const index=layers.findIndex(item=>item.name===data.layer.name);if(index>=0)layers[index]=data.layer;else layers.push(data.layer);download.href=data.download_url;download.style.display='inline-block';preview.dataset.layer=data.layer.name;preview.style.display='inline-block'}}else{if(!response.ok)throw new Error('WPS 실행 실패');const blob=await response.blob();wpsObjectUrl=URL.createObjectURL(blob);const disposition=response.headers.get('content-disposition')||'',match=disposition.match(/filename=\"?([^\";]+)\"?/i);download.href=wpsObjectUrl;download.download=match?match[1]:(process.replace('.','_')+(contentType.includes('tiff')?'.tif':'.geojson'));download.style.display='inline-block';result.value=`완료\\n프로세스: ${process}\\n파일 크기: ${(blob.size/1024).toFixed(1)} KB`}}catch(error){result.value='오류: '+error.message}finally{button.disabled=false}};
...
async function previewLayer(name){const l=layers.find(x=>x.name===name);if(!window.L)return alert('Leaflet을 불러오지 못했습니다. 인터넷 연결을 확인해 주세요.');activePreview=name;activePreviewFilter='';document.querySelector('#previewFilter').value='';document.querySelector('#previewFilterHint').textContent='';if(l.type!=='raster')fetch('/api/layers/'+encodeURIComponent(name)+'/fields').then(response=>response.json()).then(data=>{if(data.fields)document.querySelector('#previewFilterHint').textContent='사용 가능한 필드: '+data.fields.join(', ')}).catch(()=>{});
...
async function changePreviewService(){if(!activePreview||!map)return;const l=layers.find(x=>x.name===activePreview),mode=document.querySelector('#previewService').value;activePreviewMode=mode;document.querySelector('#previewFilterBar').style.display=l.type!=='raster'&&['WMS','WMS_TILED'].includes(mode)?'flex':'none';clearPreviewLayers();document.querySelector('#previewMeta').textContent=`${l.name} · ${l.type} · ${mode} 미리보기`;
